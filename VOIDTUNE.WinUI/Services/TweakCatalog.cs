...
public static class TweakCatalog
{
    ...
    private static readonly Tweak[] Baseline =
    {
        ...
        new() { Id="cpu8", Category="CPU", Tier=TweakTier.Safe, Name="DPC Watchdog Tune", Description="Reduce deferred-procedure-call delays.",
            ApplyCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Kernel"" /v DpcWatchdogProfileOffset /t REG_DWORD /d 0 /f && reg add ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Kernel"" /v DpcTimeout /t REG_DWORD /d 0 /f", RevertCmd=@"reg delete ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Kernel"" /v DpcWatchdogProfileOffset /f & reg delete ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Kernel"" /v DpcTimeout /f & exit /b 0" },
        ...
        new() { Id="ram2", Category="RAM", Tier=TweakTier.Safe, Name="Large System Cache", Description="Increase the system file cache in RAM.",
            ApplyCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management"" /v LargeSystemCache /t REG_DWORD /d 1 /f", RevertCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management"" /v LargeSystemCache /t REG_DWORD /d 0 /f" },
        ...
        new() { Id="ram4", Category="RAM", Tier=TweakTier.Extreme, Name="No Memory Compression", Description="Disable RAM compression overhead.",
            ApplyCmd="PS:Disable-MMAgent -MemoryCompression", RevertCmd="PS:Enable-MMAgent -MemoryCompression" },
        ...
        new() { Id="ram6", Category="RAM", Tier=TweakTier.Safe, Name="No Working-Set Aging", Description="Keep active process pages in RAM longer.",
            ApplyCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management"" /v DisableWorkingSetTrimming /t REG_DWORD /d 1 /f", RevertCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management"" /v DisableWorkingSetTrimming /t REG_DWORD /d 0 /f" },
        ...
        new() { Id="net14", Category="Network", Tier=TweakTier.Safe, Name="CTCP", Description="Compound TCP congestion algorithm for high-bandwidth links.",
            ApplyCmd="netsh int tcp set global congestionprovider=ctcp", RevertCmd="netsh int tcp set global congestionprovider=default" },
        ...
        new() { Id="deb6", Category="Debloat", Tier=TweakTier.Safe, Name="Raw Mouse", Description="Disable pointer acceleration.",
            ApplyCmd=@"reg add ""HKCU\Control Panel\Mouse"" /v MouseSpeed /t REG_SZ /d 0 /f", RevertCmd=@"reg add ""HKCU\Control Panel\Mouse"" /v MouseSpeed /t REG_SZ /d 1 /f" },
        ...
        new() { Id="appx1", Category="Processes", Tier=TweakTier.Safe, Name="Hide Security Tray Icon", Description="Stop SecurityHealthSystray.exe at logon. Windows Defender protection stays fully ON — only the tray icon process is removed.",
            ApplyCmd=@"reg delete ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"" /v SecurityHealth /f", RevertCmd=@"reg add ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"" /v SecurityHealth /t REG_EXPAND_SZ /d ""%windir%\system32\SecurityHealthSystray.exe"" /f" },
        new() { Id="appx2", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Bing News & Weather", Description="Uninstall the Bing News and Weather apps — kills their background tile/feed updaters.",
            ApplyCmd="PS:Get-AppxPackage -Name '*Microsoft.BingNews*','*Microsoft.BingWeather*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Microsoft.BingNews*','*Microsoft.BingWeather*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx3", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Phone Link", Description="Uninstall Phone Link (YourPhone) — removes PhoneExperienceHost background process.",
            ApplyCmd="PS:Get-AppxPackage -Name '*Microsoft.YourPhone*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Microsoft.YourPhone*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx4", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Groove & Movies", Description="Uninstall Groove Music (ZuneMusic) and Movies & TV (ZuneVideo).",
            ApplyCmd="PS:Get-AppxPackage -Name '*Microsoft.ZuneMusic*','*Microsoft.ZuneVideo*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Microsoft.ZuneMusic*','*Microsoft.ZuneVideo*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx5", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Teams (Chat)", Description="Uninstall the consumer Teams / Chat app — removes its background host.",
            ApplyCmd="PS:Get-AppxPackage -Name '*MicrosoftTeams*','*MSTeams*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*MicrosoftTeams*','*MSTeams*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx6", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Clipchamp", Description="Uninstall the Clipchamp video editor app.",
            ApplyCmd="PS:Get-AppxPackage -Name '*Clipchamp.Clipchamp*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Clipchamp.Clipchamp*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx7", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Help & Get Started", Description="Uninstall the Get Help and Tips/Get Started apps.",
            ApplyCmd="PS:Get-AppxPackage -Name '*Microsoft.GetHelp*','*Microsoft.Getstarted*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Microsoft.GetHelp*','*Microsoft.Getstarted*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx8", Category="Processes", Tier=TweakTier.Extreme, Name="Remove People & Feedback", Description="Uninstall the People app and the Feedback Hub.",
            ApplyCmd="PS:Get-AppxPackage -Name '*Microsoft.People*','*Microsoft.WindowsFeedbackHub*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Microsoft.People*','*Microsoft.WindowsFeedbackHub*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx9", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Maps, To Do, Office Hub", Description="Uninstall Windows Maps, Microsoft To Do and the Office hub stub.",
            ApplyCmd="PS:Get-AppxPackage -Name '*Microsoft.WindowsMaps*','*Microsoft.Todos*','*Microsoft.MicrosoftOfficeHub*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*Microsoft.WindowsMaps*','*Microsoft.Todos*','*Microsoft.MicrosoftOfficeHub*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        new() { Id="appx10", Category="Processes", Tier=TweakTier.Extreme, Name="Remove Cortana & Mixed Reality", Description="Uninstall the Cortana app and the Mixed Reality Portal.",
            ApplyCmd="PS:Get-AppxPackage -Name '*549981C3F5F10*','*Microsoft.MixedReality.Portal*' | Remove-AppxPackage -EA SilentlyContinue", RevertCmd="PS:Get-AppxPackage -AllUsers -Name '*549981C3F5F10*','*Microsoft.MixedReality.Portal*' | ForEach-Object { Add-AppxPackage -DisableDevelopmentMode -Register (Join-Path $_.InstallLocation 'AppXManifest.xml') -EA SilentlyContinue }" },
        ...
        new() { Id="gm7", Category="Game", Tier=TweakTier.Extreme, Name="Disable HPET", Description="Disable High Precision Event Timer. Can lower latency on some CPUs.",
            ApplyCmd="bcdedit /set disabledynamictick yes & bcdedit /deletevalue useplatformclock & exit /b 0", RevertCmd="bcdedit /set disabledynamictick no & bcdedit /set useplatformclock true & exit /b 0" },
        ...
        new() { Id="gm9", Category="Game", Tier=TweakTier.Safe, Name="MSI Mode All Devices", Description="Enable Message Signaled Interrupts on capable PCI/USB devices.",
            ApplyCmd=@"PS:Get-WmiObject Win32_PnPSignedDriver | Where-Object { $_.DeviceClass -match 'Display|Net|USB|Media' } | ForEach-Object { $path = ""HKLM:\SYSTEM\CurrentControlSet\Enum\$($_.DeviceID)\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties""; if (Test-Path (Split-Path $path)) { New-Item $path -Force | Out-Null; Set-ItemProperty $path -Name MSISupported -Value 1 } }", RevertCmd="" },
        ...
        new() { Id="stor6", Category="Storage", Tier=TweakTier.Extreme, Name="Disable NTFS Journal", Description="Delete the NTFS change journal on C:. Breaks VSS shadow copies.",
            ApplyCmd="fsutil usn deletejournal /D C:", RevertCmd="fsutil usn createjournal m=0x10000000 a=0x800000 C:" },
        ...
        new() { Id="p2", Category="Privacy", Tier=TweakTier.Safe, Name="Block Ad Delivery", Description="Stop Microsoft advertising content delivery.",
            ApplyCmd=@"reg add ""HKCU\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager"" /v ContentDeliveryAllowed /t REG_DWORD /d 0 /f", RevertCmd=@"reg add ""HKCU\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager"" /v ContentDeliveryAllowed /t REG_DWORD /d 1 /f" },
        new() { Id="p3", Category="Privacy", Tier=TweakTier.Safe, Name="Deny Camera", Description="Block all apps from the webcam.",
            ApplyCmd=@"reg add ""HKCU\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\webcam"" /v Value /t REG_SZ /d Deny /f", RevertCmd=@"reg add ""HKCU\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\webcam"" /v Value /t REG_SZ /d Allow /f" },
        new() { Id="p4", Category="Privacy", Tier=TweakTier.Safe, Name="Deny Microphone", Description="Block all apps from the microphone.",
            ApplyCmd=@"reg add ""HKCU\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\microphone"" /v Value /t REG_SZ /d Deny /f", RevertCmd=@"reg add ""HKCU\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\microphone"" /v Value /t REG_SZ /d Allow /f" },
        ...
        new() { Id="rst5", Category="Restore", Tier=TweakTier.Safe, Name="Restore Mouse", Description="Re-enable pointer acceleration.",
            ApplyCmd=@"reg add ""HKCU\Control Panel\Mouse"" /v MouseSpeed /t REG_SZ /d 1 /f", RevertCmd="" },

        // ── NUCLEAR ───────────────────────────────────────────────────────────
        new() { Id="nuke1", Category="Nuclear", Tier=TweakTier.Nuclear, Name="Disable Defender RT", Description="Disable Defender real-time protection (Tamper Protection must be OFF). MAJOR security risk.",
            ApplyCmd=@"reg add ""HKLM\SOFTWARE\Policies\Microsoft\Windows Defender"" /v DisableAntiSpyware /t REG_DWORD /d 1 /f & reg add ""HKLM\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection"" /v DisableRealtimeMonitoring /t REG_DWORD /d 1 /f & exit /b 0", RevertCmd=@"reg delete ""HKLM\SOFTWARE\Policies\Microsoft\Windows Defender"" /v DisableAntiSpyware /f & reg delete ""HKLM\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection"" /v DisableRealtimeMonitoring /f & exit /b 0" },
        new() { Id="nuke2", Category="Nuclear", Tier=TweakTier.Nuclear, Name="Disable SmartScreen", Description="Disable SmartScreen app/file reputation checks.",
            ApplyCmd=@"reg add ""HKLM\SOFTWARE\Policies\Microsoft\Windows\System"" /v EnableSmartScreen /t REG_DWORD /d 0 /f", RevertCmd=@"reg add ""HKLM\SOFTWARE\Policies\Microsoft\Windows\System"" /v EnableSmartScreen /t REG_DWORD /d 1 /f" },
        new() { Id="nuke3", Category="Nuclear", Tier=TweakTier.Nuclear, Name="Disable UAC", Description="Disable User Account Control prompts. Requires reboot.",
            ApplyCmd=@"reg add ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"" /v EnableLUA /t REG_DWORD /d 0 /f", RevertCmd=@"reg add ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"" /v EnableLUA /t REG_DWORD /d 1 /f" },
        new() { Id="nuke4", Category="Nuclear", Tier=TweakTier.Nuclear, Name="Disable Core Isolation", Description="Disable memory integrity / HVCI. Recovers CPU overhead. Requires reboot.",
            ApplyCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\HypervisorEnforcedCodeIntegrity"" /v Enabled /t REG_DWORD /d 0 /f", RevertCmd=@"reg add ""HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\HypervisorEnforcedCodeIntegrity"" /v Enabled /t REG_DWORD /d 1 /f" },
        new() { Id="nuke5", Category="Nuclear", Tier=TweakTier.Nuclear, Name="Disable Firewall", Description="Turn off Windows Firewall on all profiles. Only behind a trusted router.",
            ApplyCmd="netsh advfirewall set allprofiles state off", RevertCmd="netsh advfirewall set allprofiles state on" },
    };
}
