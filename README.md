...
|---------|---------|----------|-----------------|
| 0.8.18 | GPL v3 | Windows 10/11 (x64) | C# / WinUI 3 (Windows App SDK) |
...
## 🚀 What's New in v0.8.18 (WinUI 3 edition)

- **Fixed PS: tweaks that could fail with "the term '...ps1' is not recognized"** — field-confirmed real bug: PowerShell was invoked by bare name and could resolve to something that isn't real PowerShell on some systems. Now always uses the fully-qualified System32 path
- **Fixed reboot-gated tweaks flipping off while scrolling** — a remaining gap in 0.8.15's container-recycling fix; added a short re-validation delay so a transient recycling artifact can no longer be mistaken for a real toggle click

> Full history in [CHANGELOG.md](CHANGELOG.md).

## 🚀 What's New in v0.8.17 (WinUI 3 edition)

- **Real installer wizard** — the MSI previously had zero authored UI. Added the standard Welcome → License → Progress → Exit wizard with a **"Launch VOIDTUNE" checkbox on finish** (checked by default), plus graceful auto-close of a running VOIDTUNE before installing
- **In-app updates now auto-launch VOIDTUNE when they finish** — the MSI update path used to require clicking through the wizard again and never reopened the app; both update paths (MSI and portable) now behave identically: silent, no extra clicks, auto-relaunch

> Full history in [CHANGELOG.md](CHANGELOG.md).

## 🚀 What's New in v0.8.16 (WinUI 3 edition)

- **Fixed extreme lag when applying tweaks** — a self-inflicted regression from 0.8.14's own self-healing fallback feature: failed tweaks were retried one at a time in a blocking loop instead of concurrently, so any run with a handful of legitimate failures added many seconds of silent hang with no progress-bar feedback. Fallback retries now run through the same bounded-parallelism batching as everything else.

> Full history in [CHANGELOG.md](CHANGELOG.md).

## 🚀 What's New in v0.8.15 (WinUI 3 edition)

- **Fixed a real data-integrity bug** — toggling one tweak could silently apply/revert a *different* tweak while scrolling the Tweaks/Startup/Customization lists, caused by a container-recycling race between two separate WinUI bindings. Field-reported as "toggles change tweaks that already had a state." Fixed by reading the row's DataContext instead of a separately-bound Tag, which can't go stale the same way.
...
1. Download **`VOIDTUNE-0.8.18-Setup.msi`** from the [Releases](https://github.com/otzpt/VOIDTUNE/releases) page.
...
1. Download **`VOIDTUNE-0.8.18-portable-win-x64.zip`** from [Releases](https://github.com/otzpt/VOIDTUNE/releases).
